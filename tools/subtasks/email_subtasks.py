...
def get_events_for_days(date_strs: list[str]):
    """
    Retrieves events for a list of days in São Paulo time.
    If you want to check for multiple days, call this with multiple inputs.
    ...
    for date_str in date_strs:
        ...
        # Create start and end of the day in São Paulo timezone
        start_of_day = sao_paulo_tz.localize(datetime.combine(day, time.min)).isoformat()
        end_of_day = sao_paulo_tz.localize(datetime.combine(day, time.max)).isoformat()
...
def format_datetime_with_timezone(dt_str, timezone="America/Sao_Paulo"):
    """
...
def send_calendar_invite(
    emails, title, start_time, end_time, email_address, timezone="America/Sao_Paulo"
):
    creds = get_credentials(None, None)
    service = build("calendar", "v3", credentials=creds)
...
def send_email_to_address(
    ...
    # Correção aqui: o corpo da mensagem deve ter apenas o campo 'raw'
